...
AI Code Reviewer is a GitHub Action that leverages Google's Gemini 2.0 Flash API to provide intelligent feedback and suggestions on
your pull requests. This powerful tool helps improve code quality and saves developers time by automating the code
review process.

## Features

- Reviews pull requests using Google's Gemini 2.0 Flash API
...
1. To use this GitHub Action, you need a Google API key. If you don't have one, sign up for an API key
   at [Google AI Studio](https://makersuite.google.com/app/apikey).

2. Add the Google API key as a GitHub Secret in your repository with the name `GOOGLE_API_KEY`. You can find more
...
The AI Code Reviewer GitHub Action retrieves the pull request diff, filters out excluded files, and sends all code changes to
the Gemini API in a single request. It then generates review comments based on the AI's response and adds them to the pull request.
...
sequenceDiagram
    ...
    GitHub_Action->>Gemini: Send all code changes in a single request
    Gemini-->>GitHub_Action: Generate review comments
